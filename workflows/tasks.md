...
- **Dual-Compatible 3-Column Telemetry Callout**: Conclude with a `> [!TIP]` callout titled `### 💡 Next Recommended Step` featuring the structured 3-column telemetry matrix (`| Milestone Progress | Active Task | System Health |` with `[■■■■□□□□□□]`) recommending `pk:test` or implementing the first task. When multiple next steps exist, invoke native interactive selection tools with Option 1 `(Recommended)` so the developer can confirm with `Enter`.
