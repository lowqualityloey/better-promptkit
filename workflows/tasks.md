...
- **Dual-Compatible Visual Callout**: Conclude with a `> [!TIP]` callout titled `### 💡 Next Recommended Step:` featuring the situational progress bar (`[■■■■□□□□□□]`) recommending `pk:test` (upfront test strategy) or implementing the first task.
